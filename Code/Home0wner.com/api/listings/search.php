...
$whereClause = [];
$comparisonFields = array("bedrooms", "bathrooms", "sq_feet", "price");

foreach ($_GET as $key => $value) {
    if ($key != "action" ) {
        if (!empty($value)) {
            if (filter_var($value, FILTER_VALIDATE_INT)) {
                if (in_array($key, $comparisonFields)) {
